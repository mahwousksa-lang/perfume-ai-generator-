...
export default function Home() {
  ...
  // ── LoRA ───────────────────────────────────────────────────────────────────────
  const [loraModelId, setLoraModelId] = useState(process.env.NEXT_PUBLIC_DEFAULT_LORA_MODEL ?? '');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setProgressStep('نقوم ببناء المشهد ثلاثي الأبعاد...');
      ...
      // ── Step 2: Generate captions (non-blocking) ───────────────────────────────
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        />
        <div
          className="absolute bottom-[-10%] left-[-15%] w-[500px] h-[500px] rounded-full opacity-[0.03]"
      ...
      <header className="relative z-10 border-b border-[var(--obsidian-border)] bg-[rgba(8,8,16,0.8)] backdrop-blur-xl sticky top-0">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-display text-xl font-semibold text-gold-shimmer">
                Perfume Brand AI
              </h1>
                ...
                <div key={item.label} className="flex flex-col items-center gap-2">
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center text-lg
                                border border-[var(--obsidian-border)] bg-[var(--obsidian-card)]
                                animate-pulse"
          ...
          <div className={`grid gap-8 ${appStep === 'output' ? 'grid-cols-[380px,1fr]' : 'grid-cols-[420px,1fr]'}`}>
            ...
            <div className="space-y-5">
                  ...
                  {perfumeData.name && (
                    <div className="mt-4 pt-4 border-t border-[var(--obsidian-border)] space-y-2">
                      ...
                      <div className="bg-black/20 rounded-xl p-3 space-y-1">
                        ...
                        {perfumeData.notes && (
                          <p className="text-xs text-[var(--text-muted)] line-clamp-2 font-arabic">
                            {perfumeData.notes}
                          </p>
                        )}
                      </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="مثال: Bleu de Chanel"
                    ...
                    <div>
                      ...
                      <select
                        className="luxury-input"
                        value={perfumeData.gender ?? 'men'}
                        onChange={(e) =>
                          setPerfumeData((p) => ({
                            ...p,
                            gender: e.target.value as PerfumeData['gender'],
                          }))
                        }
                      >
                    ...
                    <div>
                      <label className="text-xs text-[var(--text-muted)] mb-1.5 block">
                        الملاحظات العطرية (اختياري)
                      </label>
                      <input
                        type="text"
                        className="luxury-input"
                        placeholder="مثال: عود، مسك، ورد، عنبر..."
              ...
              <LoraConfig
                ...
                onLoraModelChange={setLoraModelId}
              ...
              <p className="text-[11px] text-[var(--text-muted)] text-center leading-relaxed">
                ...
                <br />
                عبر Replicate API — قد يستغرق 60–120 ثانية
              </p>
            </div>
      ...
      <footer className="relative z-10 border-t border-[var(--obsidian-border)] mt-20 py-6">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          <p className="text-xs text-[var(--text-muted)]">
            Powered by Replicate · Anthropic Claude · Next.js
          </p>
          <p className="text-xs text-[var(--text-muted)]">
            © {new Date().getFullYear()} Perfume Brand AI
          </p>
        </div>
      </footer>
    </div>
