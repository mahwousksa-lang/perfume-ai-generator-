...
export function buildPlatformSpecificPrompt(params: {
  ...
  // Character description based on attire
...
RULE 3: The character must have black swept-back hair, thick black beard
...
// ── وصف الشخصية حسب المنصة ──────────────────────────────────────────────────

function getCharacterForFormat(attire: string, format: 'story' | 'post' | 'landscape'): string {
  const baseFeatures = 'black swept-back hair, thick full black beard, confident charming expression';

  if (format === 'story') {
    // شبابي — ملابس عصرية أو ثوب أنيق
    if (attire.includes('thobe') || attire.includes('bisht')) {
      return `A stylish young Arab man with ${baseFeatures}, rendered in 3D Pixar/Disney CGI style. He wears a pristine white Saudi thobe with a modern slim fit, looking trendy and youthful.`;
    }
    return `A stylish young Arab man with ${baseFeatures}, rendered in 3D Pixar/Disney CGI style. He wears a sleek black leather jacket over a crisp white t-shirt, modern streetwear luxury style, looking cool and confident.`;
  }

  if (format === 'post') {
    // أنيق كلاسيكي — بدلة أو بشت
    if (attire.includes('thobe') || attire.includes('bisht')) {
      return `A distinguished Arab man with ${baseFeatures}, rendered in 3D Pixar/Disney CGI style. He wears a pristine white Saudi thobe with an elegant golden bisht (ceremonial cloak), looking regal and sophisticated.`;
    }
    return `A distinguished Arab man with ${baseFeatures}, rendered in 3D Pixar/Disney CGI style. He wears an elegant black suit with subtle gold trim details on the lapels, crisp white shirt, gold silk tie, and a gold pocket square, looking sophisticated and powerful.`;
  }

  // Landscape — خبير عطور
  if (attire.includes('thobe') || attire.includes('bisht')) {
    return `A knowledgeable Arab perfume expert with ${baseFeatures}, rendered in 3D Pixar/Disney CGI style. He wears a clean white Saudi thobe, looking scholarly and authoritative, like a trusted perfume reviewer.`;
  }
  return `A knowledgeable Arab perfume expert with ${baseFeatures}, rendered in 3D Pixar/Disney CGI style. He wears a dark navy blazer over a light blue shirt (no tie), sleeves slightly rolled up, looking approachable and knowledgeable, like a trusted perfume reviewer.`;
}
